...
export const createAiDeck = async (deckName, cards, userId) => {
    try {
        // No need to fetch current user again; use the passed userId directly

        // Generate a unique ID for the deck
        const deckId = ID.unique();

        // Create the deck and associate it with the user
        const newDeck = await databases.createDocument(
            appwriteConfig.databaseId,
            appwriteConfig.deckCollectionId,
            deckId,
            {
                ...
                userId, // Use the userId passed in as an argument
                deckName, // Deck name provided
            }
        );

        // Create all cards and associate them with the deck
        await Promise.all(
            cards.map((card) =>
                createCard(deckId, card.cardName, card.frontText, card.backText) // Assuming createCard handles card creation
            )
        );

        return newDeck; // Return the created deck
    } catch (error) {
